...
struct BehaviorsView: View {
    ...
    let behaviors = Bundle.main.decode([BehaviorObj].self, from: "behaviors.json")
    
    var body: some View {
        VStack{
            List {
                ForEach(behaviors, id: \.self) {behavior in
                    NavigationLink(destination: BehaviorItemView(title: behavior.name, content: behavior.description)){
                        Text(behavior.name)
                    }
                }
